# Internal Telemetry Live Source Contract v0.2
...
revision_source: OPUS_REVIEW_INTERNAL_TELEMETRY_LIVE_SOURCE_CONTRACT_V0_1_20260705.md
...
8. The maximum claim level is assigned using the claim ladder in this contract and only after the proof protocol in this contract has passed.
...
|---|---|---|
| `telemetry_event_id` | Stable event identity | Unique within source scope; no display claim if absent unless another stable key is documented. If a substitute stable key replaces `telemetry_event_id`, the substitution must be recorded in the source badge or review receipt. |
...
| `PROVIDER_BACKED_TELEMETRY_VERIFIED` | `PROVIDER_API_SYNC` | Provider/API receipt, ingestion mapping, source/display reconciliation, freshness metadata | Full completeness beyond documented provider/source scope; continuous liveness, monitoring, alerting, or billing-verified cost unless separate proof exists |
...
- If `cost_estimate` is displayed, the formula/source is named and no billing-verified claim is made unless provider-billed evidence is attached.
...
| Preserved unresolved counts | Baseline `26959`, export `26957`, delta `2`, `gate_claim_mapping_missing = 18019`, and `66.84%` remain marked unresolved/source-limited in any downstream view until category recount proof exists | Block validated-count or category-ranking claim |
...
- Declared claim level exceeds the proof protocol evidence in this contract.
