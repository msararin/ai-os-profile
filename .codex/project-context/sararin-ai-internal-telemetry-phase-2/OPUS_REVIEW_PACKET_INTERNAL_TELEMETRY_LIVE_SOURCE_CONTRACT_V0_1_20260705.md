# Opus Review Packet: Internal Telemetry Live Source Contract v0.1

You are reviewing a telemetry source contract specification. This is provider-backed contract review only. Do not treat this review as provider-backed telemetry verification. Reviewing this spec is not the same as verifying telemetry data.

Evaluate whether the contract is sufficient for:
1. source_mode clarity
2. required event fields
3. optional model/provider fields
4. freshness metadata
5. source badge rules
6. claim ladder
7. proof protocol before each claim level
8. privacy/redaction rules
9. data quality checks before visualization
10. stop conditions
11. staged remediation path
12. prevention of overclaiming
13. readiness for SQLite staging proof plan
14. readiness for later production/provider-backed source verification

Return a structured review with:
- reviewer/provider/model metadata if available
- input artifact reviewed
- verdict using exactly one of: ACCEPT, ACCEPT_WITH_REVISIONS, REVISE_BEFORE_IMPLEMENTATION, REJECT_UNSAFE_TO_IMPLEMENT
- major findings
- required revisions
